...
- [x] `V1-21` Implement `submit_pr(summary)` handling and enqueue merge request metadata. Test: submit call creates queue entry.
...
- [x] `V1-35` Add end-to-end V1 happy-path integration test (`spec -> areas -> tickets -> done`). Test: one fixture repo completes full flow.
...
- [ ] `V1-36` Add idle-on-empty-spec guard to run loop: if `spec.md` is missing or contains
...
- [ ] `V1-37` Implement `runArchitectAreas(repoPath)`: open plan worktree, build architect
...
- [ ] `V1-38` Implement `runManagerTickets(repoPath)`: for each area with no open or
...
- [ ] `V1-39` Update `runOrchestratorLoop` to call steps in order: spec guard → area
...
## Next Integration Test Batch

- [x] `IT-01` Agent-to-queue integration with fixture harness. Test: fake codex output containing `submit_pr("...")` creates exactly one queue file with ticket/branch/worktree/summary metadata.
- [x] `IT-02` Queue success end-to-end with real branch merge. Test: passing fixture processes one queue item, moves ticket `in-progress -> done`, removes queue entry, and makes ticket commit reachable from `master`.
- [x] `IT-03` Queue failure end-to-end with test failure. Test: failing fixture processes one queue item, moves ticket `in-progress -> open`, removes queue entry, and appends failure note text.
- [x] `IT-04` Single-flight queue ordering with two pending items. Test: one processor invocation completes only the oldest item and leaves the second item pending.
- [x] `IT-05` Merge conflict handling path. Test: conflicting `master` change causes merge step failure, reopens ticket, and records merge failure output.
- [x] `IT-06` Orchestrator tick ordering integration. Test: `runOrchestratorForTicks` processes merge queue before assigning additional open tickets.
- [x] `IT-07` Strict integration prerequisites behavior. Test: integration run fails with clear error when codex binary is missing and fails with clear error when both API keys and OAuth auth are missing.
- [x] `IT-08` Recovery/idempotence after interrupted queue processing. Test: rerunning queue processor after partial failure converges to one valid ticket state with no duplicate transitions.
- [x] `IT-09` Red-master assignment gate behavior. Test: one orchestrator tick with failing master leaves oldest ticket in `open/` and performs no assignment transition.
- [x] `IT-10` Global red-master halt and recovery behavior. Test: pending merge item is not processed while master is red, then processes successfully after master health is restored.
