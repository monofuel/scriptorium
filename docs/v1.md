...
- [ ] `IT-07` Strict integration prerequisites behavior. Test: integration run fails with clear error when codex binary is missing and fails with clear error when API keys are missing.
- [ ] `IT-08` Recovery/idempotence after interrupted queue processing. Test: rerunning queue processor after partial failure converges to one valid ticket state with no duplicate transitions.
