...
- [x] `CA-09` Add integration tests in `tests/integration_*.nim` that run real codex on low-cost models (for example GPT-5.1 mini) behind env-key checks. Test: `make integration-test` runs when keys are present and skips cleanly when absent.
